# fix-a73f00c8f6e04523 — Spec mismatch: env var is BOT_TOKEN, not ROLLDELUXE_BOT_TOKEN

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 RDX

`docs/spec.md` line 63 mandates `ROLLDELUXE_BOT_TOKEN` as the token env var, but `src/index.ts:3` reads `process.env.BOT_TOKEN`. The `.env.example` also uses `BOT_TOKEN`. If the spec is the source of truth, the bot cannot start in the specified environment; if the spec is stale, it should be updated.


## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-a73f00c8f6e04523.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-a73f00c8f6e04523.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-a73f00c8f6e04523.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
